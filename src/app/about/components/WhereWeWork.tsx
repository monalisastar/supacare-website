...
export default function WhereWeWork() {
  ...
  return (
    <section
      className="py-20 px-6 md:px-20 flex flex-col md:flex-row items-center max-w-7xl mx-auto gap-12 transition-colors duration-500"
      style={{ backgroundColor: "limegreen" }} // ✅ switched from white to lime green
    >
      {/* Text side */}
      <div className="flex-1 max-w-lg text-center md:text-left text-white">
        <h2 className="text-3xl font-bold mb-4 drop-shadow-md">
          Local Roots, Expanding Reach
        </h2>
        <p className="mb-6 text-base md:text-lg drop-shadow-sm">
      ...
      <div className="flex-1 max-w-md relative">
        ...
        {pins.map(({ name, top, left, description }) => (
          <button
            ...
            aria-label={name}
          >
            <LeafPin color="#ffffff" /> {/* white pin contrasts nicely with lime background */}
            {hovered === name && (
              <div
                id={`tooltip-${name}`}
                className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-green-900 text-white text-xs rounded-md p-1 whitespace-nowrap shadow-lg pointer-events-none select-none"
              >
                {name}: {description}
              </div>
            )}
