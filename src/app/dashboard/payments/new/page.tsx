...
const paymentMethods = [
  ...
  },
  {
    ...
    icon: (
      <Image
        src="/mpesa.svg" // ensure this file exists in /public
        alt="M-Pesa"
        width={40}
        height={40}
      />
...
export default function NewPaymentPage() {
  ...
  const [paymentType, setPaymentType] = useState("FUNDING");
  ...
  const handleSubmit = async () => {
    ...
    try {
      const endpoint =
        paymentType === "FUNDING" ? "/api/payments/funding" : "/api/payments/new";
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          description: `${paymentType} via ${selectedMethod}`,
        }),
      });
      ...
      if (res.ok) {
        toast.success(`Payment created! ID: ${data?.fundingId || data?.id}`);
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-blue-950 via-blue-900 to-blue-800 text-white">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        ...
        >
          <h1 className="text-3xl font-bold mb-2 text-center">💳 New Payment</h1>
          <p className="text-blue-100 text-center mb-8">
            Make a payment for your project, milestone, or fund your wallet securely.
          </p>
          ...
          <div className="space-y-4 mb-6">
            ...
            <div>
              <label className="block text-blue-100 mb-1 font-medium">Payment Type</label>
              <select
                className="w-full p-2 rounded-lg bg-blue-950 border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                value={paymentType}
            ...
            {(paymentType === "PROJECT" || paymentType === "MILESTONE") && (
              <div>
                <label className="block text-blue-100 mb-1 font-medium">Entity ID</label>
                <input
                  type="text"
                  className="w-full p-2 rounded-lg bg-white/10 border border-white/20 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-blue-400"
                  value={entityId}
